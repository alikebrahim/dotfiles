...
│   ├── run_after_ensure-tmux-plugins.sh             <- --ensure port (DECIDED D7-A)
...
`create_` seeds — both mechanisms are spike-validated (U2 encrypted
fragments compose; U8 `create_` seeds behave); the final pick happens at
Phase 2 authoring, since authorized_keys is mostly read-only operational
data either way.
...
drift/apply loops). **Spike-verified (U8, 2026-08-18): status/diff stay
quiet after ssh rewrites the file, and deletion→apply recreates it with the
seed. Caveat: the `create_` source must be NON-EMPTY (a comment line) — an
empty source silently never materializes.**
...
- `encrypted_dot_ssh/config.tmpl`? — no: encryption applies to the *source
  ...
  `.chezmoiignore`... both shapes were candidates before the spike; the
  spike settled it (U2, 2026-08-18): the simple shape works — one
  `encrypted_dot_ssh/config.tmpl` whose plaintext (after age decrypt) is a
  Go template — attributes and `.tmpl` compose, with decrypt happening
  first and render second. Fallback if composition
...
- Bootstrap: docs-endorsed pattern — commit a passphrase-encrypted
  ...
  re-runs after key rotation — the official FAQ pattern.) **Spike note
  (U4/U5, 2026-08-18): the `[age]` identity/recipient settings must be
  generated by `.chezmoi.toml.tmpl` itself — re-running `init` re-renders
  the whole config from the template and wipes manually edited `[age]`
  sections.**
...
| `install-tmux-plugins.sh` | Option A: `run_after_ensure-tmux-plugins.sh` (port: `--ensure` semantics); Option B: `.chezmoiexternal` `git-repo` entries (v2.50+) with pins + refreshPeriod | apply (A) / refresh (B) | DECIDED D7-A (2026-08-17): the `run_after_` ensure-script; Option B rejected (diff-invisible, timer-driven); explicit `--update` command stays manual |
