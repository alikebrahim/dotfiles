...
-- Every mouse path uses the same global workspace coordinator as the keyboard
-- and Quickshell bridge. Right-click remains a view alias; independent tag
-- toggles are incompatible with the one-logical-workspace contract.
local function taglist_buttons()
    return gears.table.join(
        awful.button({}, 1, function(t) workspace.view_index(t.index) end),
        awful.button({ beautiful.modkey or "Mod4" }, 1, function(t)
            if client.focus then workspace.move_client_to_index(client.focus, t.index, true) end
        end),
        awful.button({}, 3, function(t) workspace.view_index(t.index) end),
        awful.button({ beautiful.modkey or "Mod4" }, 3, function(t)
            if client.focus then workspace.move_client_to_index(client.focus, t.index, true) end
        end),
        awful.button({}, 4, function() workspace.view_relative(-1) end),
        awful.button({}, 5, function() workspace.view_relative(1) end)
    )
end
